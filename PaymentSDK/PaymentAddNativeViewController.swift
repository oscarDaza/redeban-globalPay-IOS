...
open class PaymentAddNativeViewController: UIViewController {
    ...
    let PaymentCardR:PaymentCard = PaymentCard()
    ...
    private let PaymentLogo : UIImageView = {
        ...
        imageView.image = UIImage(named:"logo_payment_color", in: Bundle(for: PaymentCard.self), compatibleWith: nil)
    ...
    var cardType:PaymentCardType =  PaymentCardType.notSupported {
        didSet {
            ...
            self.PaymentCardR.cardType = self.cardType
     ...
     @objc open func getValidCard()->PaymentCard?
    {
        ...
        if self.PaymentCardR.cardType == .notSupported{
            return nil
        }
        guard let _ = self.PaymentCardR.cardHolder else {
            self.nameField.errorMessage = "Invalid".localized
            return nil
        }
        guard let _ = self.PaymentCardR.cardNumber else {
        ...
        if self.cardType == .alkosto || self.cardType == .exito  //tarjetas tuya
        {
            ...
            guard let _ = self.PaymentCardR.nip  else {
                 self.nipField.errorMessage = "Invalid".localized
                return nil
            }
            guard let _ = self.PaymentCardR.fiscalNumber else {
                 self.documentField.errorMessage = "Invalid".localized
                return nil
            }
            
            return self.PaymentCardR
            
        }else { // las demás
            guard let _ = self.PaymentCardR.cvc else {
                 self.cvcField.errorMessage = "Invalid".localized
                return nil
            }
            guard let _ = self.PaymentCardR.expiryMonth else {
                 self.expirationField.errorMessage = "Invalid".localized
                return nil
            }
            guard let _ = self.PaymentCardR.expiryYear else {
                 self.expirationField.errorMessage = "Invalid".localized
                return nil
            }
           return self.PaymentCardR
    ...
    @objc func scanCard(_ sender: Any) {
        PaymentSDKClient.scanCard(self) { (closed, number, expiry, cvv, card) in
            if !closed
            {
                ...
                self.PaymentCardR.cvc = self.cvcField.text
                
                self.PaymentCardR.cardNumber = self.cardField.text?.replacingOccurrences(of: "-", with: "")
                self.cardType = PaymentCard.getTypeCard((self.PaymentCardR.cardNumber)!)
                let valExp = self.expirationField.text!.components(separatedBy: "/")
                if valExp.count > 1
                {
                    let expiryYear = Int(valExp[1])! + 2000
                    let expiryMonth = valExp[0]
                    self.PaymentCardR.expiryYear =  "\(expiryYear)"
                    self.PaymentCardR.expiryMonth =  expiryMonth
                }
    ...
    public func textField(_ textField: UITextField, didFillMandatoryCharacters complete: Bool, didExtractValue value: String) {
            ...
            if complete
            {
                if PaymentCard.validateExpDate(self.expirationField.text!)
                {
                    ...
                    if valExp.count > 1
                    {
                        ...
                        self.PaymentCardR.expiryYear =  "\(expiryYear)"
                        self.PaymentCardR.expiryMonth = valExp[0]
                    }
                }
                else
                {
                    self.PaymentCardR.expiryYear = nil
                    self.PaymentCardR.expiryMonth = nil
        ...
        if textField == self.cvcField
        {
            self.cvcField.errorMessage = ""
                ...
                {
                    self.cvcField.errorMessage = "Invalid".localized
                    self.PaymentCardR.cvc = nil
                }
                else
                {
                    self.PaymentCardR.cvc = value
                }
        }
        if textField == self.documentField{
            self.PaymentCardR.fiscalNumber = self.documentField.text
        }
        if textField == self.nipField {
            self.nipField.errorMessage = ""
            if value.count != 4{
                self.nipField.errorMessage = "Invalid".localized
                self.PaymentCardR.nip = nil
            } else {
                self.PaymentCardR.nip  = self.nipField.text
            }
        ...
        if textField == self.documentField{
            ...
            if value.count > 3{
                self.PaymentCardR.fiscalNumber = value
            } else {
                self.documentField.errorMessage = "Invalid".localized
                self.PaymentCardR.fiscalNumber = nil
            }
        }
...
extension PaymentAddNativeViewController {
    ...
    @objc func textfieldDidChange(_ sender:Any){
        ...
        if sender as? SkyFloatingLabelTextField == self.nameField{
            ...
            if value.count > 4{
                self.PaymentCardR.cardHolder = value
            } else {
                self.nameField.errorMessage = "Invalid".localized
                self.PaymentCardR.cardHolder = nil
            }
        }
...
@objc public extension UIViewController {
    ...
    func addPaymentWidget(toView containerView:UIView,  delegate:PaymentCardAddedDelegate?, uid:String, email:String) -> PaymentAddNativeViewController{
        ...
        self.addChildViewController(PaymentAddVC)
        ...
        PaymentAddVC.didMove(toParentViewController: self)
        return PaymentAddVC
    }
